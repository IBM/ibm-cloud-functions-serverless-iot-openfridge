...
class TriggerStore {
  ...
  add(trigger) {
    const _insert = Promise.promisify(this.db.insert, {
      context: this.db
    });
  ...
  remove(id) {
    const _get = Promise.promisify(this.db.get, {
      context: this.db
    });
    const _destroy = Promise.promisify(this.db.destroy, {
      context: this.db
    });
  ...
  triggers(url, topic) {
    ...
    const _view = Promise.promisify(this.db.view, {
      context: this.db
    });
    const extract_triggers = body => body.rows.map(row => row.value);
    return _view('subscriptions', topic ? 'host_topic_triggers' : 'host_triggers', {
      startkey: key,
      endkey: key
    }).then(extract_triggers);
  }
